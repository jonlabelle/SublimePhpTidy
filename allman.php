...
define('CONFIGFILE', $subl_plugin_path.DIRECTORY_SEPARATOR.".phptidy-config.php");
define('CACHEFILE', '.phptidy-cache');
define('BACKUPFILE', '.phptidybak~');
define('FILESUFFIX', '.phptidy.php');
define('TEMPFILE', 'tmp.phptidy.php');
...
if (!count($files))
{
	...
	{
		echo "Error: file argument must be specified.";
		exit(1);
	}
...
{
	if (($key = array_search($file_exclude, $files)) !== false)
	{
		unset($files[$key]);
	}
}
...
foreach ($files as $key => $file)
{
	...
	if (substr($file, -12) == BACKUPFILE or substr($file, -12) == FILESUFFIX)
	{
		unset($files[$key]);
		continue;
	}
...
foreach ($files as $file)
{
	...
	{
		verbose("File contains characters which are not valid in ".$encoding.".");
	...
	do
	{
		...
		if ($count > 3)
		{
			verbose("Code processed 3 times and still not consistent!");
			break;
		}
	...
	case "replace":
		...
		$backupfile = dirname($file).(dirname($file) ? DIRECTORY_SEPARATOR : "").".".basename($file).BACKUPFILE;
...
if ($command == "replace")
{
	...
	if ($cache != $cache_orig)
	{
		...
		{
			verbose("Warning: The cache file '".CACHEFILE."' could not be saved.");
		}
	}
}
